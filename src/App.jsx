...
function App() {
  ...
  const [statusText, setStatusText] = useState("xbrowser ready - Enter a URL to start browsing");
  const addressBarRef = useRef(null);

  // Handle address bar key events
  const handleAddressBarKeydown = async (event) => {
    if (event.key === "Enter") {
  ...
  // Navigate to URL
  const navigateToUrl = async (input) => {
    try {
      let url = input;
      ...
      if (isValid) {
        ...
        try {
          ...
          setStatusText("Navigation bar injection failed - use Ctrl+I to retry");
        }
      ...
      setStatusText(`Failed to load: ${error.message}`);
  ...
  const navigateBack = async () => {
    try {
      if (browserState.historyIndex > 0) {
        ...
        addressBarRef.current.value = browserState.history[browserState.historyIndex - 1];
        setStatusText(result);
      }
    } catch (error) {
      console.error("Back navigation error:", error);
      setStatusText(`Failed to go back: ${error.message}`);
    }
  };
  ...
  const navigateForward = async () => {
    try {
      ...
      setStatusText(`Failed to go forward: ${error.message}`);
    }
  };
  ...
  const forceInjectNavigation = async () => {
    try {
      const result = await invoke("force_inject_navigation");
      setStatusText(result);
    } catch (error) {
      console.error("Force inject error:", error);
      setStatusText(`Failed to inject navigation: ${error.message}`);
    }
  };
  ...
  const toggleBookmark = async () => {
    try {
      ...
      if (existingIndex >= 0) {
        // Remove bookmark
        newBookmarks = browserState.bookmarks.filter((_, index) => index !== existingIndex);
        setStatusText("Bookmark removed");
      } else {
        // Add bookmark
        newBookmarks = [...browserState.bookmarks, { url, title, timestamp: Date.now() }];
      ...
      // Notify backend
      await invoke("update_bookmarks", { bookmarks: newBookmarks });
    } catch (error) {
...
Enter number (1-4):`);
  ...
  const closeBrowserWindow = async () => {
    try {
      ...
      setStatusText(`Failed to close browser: ${error.message}`);
    }
  };
  ...
  const isSecure = browserState.currentUrl.startsWith("https://");
  ...
  return (
    <div className="app">
      ...
      {/* Loading indicator */}
      <div className={`loading-indicator ${browserState.isLoading ? '' : 'hidden'}`}>
      ...
      <main className="content">
        <div className="welcome-area">
          <div className="welcome-content">
            ...
            <p className="welcome-subtitle">A fast, secure, and modern web browser built with Tauri + React</p>
            ...
            <div className="quick-links">
              ...
              <div className="link-grid">
                <button className="quick-link" onClick={() => handleQuickLink('https://www.google.com')}>
                  Google
                </button>
                <button className="quick-link" onClick={() => handleQuickLink('https://github.com')}>
                  GitHub
                </button>
                <button className="quick-link" onClick={() => handleQuickLink('https://tauri.app')}>
                  Tauri
                </button>
                <button className="quick-link" onClick={() => handleQuickLink('https://developer.mozilla.org')}>
                  MDN
                </button>
              </div>
            </div>
